# -*- coding: utf-8 -*-
"""
HuggingFace Dataset Metadata Extractor - Optimized Version
优化版：合并 LLM 调用 + 配置外部化
"""
...
def llm_classify_combined(dataset_id, readme_text, tags, card_data, task_categories, modalities, data_preview):
    """
    合并的 LLM 调用：同时判断榜单类型和 Agent 类型
    返回: (榜单类型, 榜单原句, Agent类型, 混合说明)
    """
    ...
    # 特殊规则：tags 含 robotics + multimodal
...
1. **榜单类型**（四选一）：
- benchmark榜单: 数据集本身是benchmark/测评基准，用于评测模型
- 名字含bench类: 仓库名含bench但README为空
- 其他榜单: 包含评测内容但不是标准benchmark
- 非榜单: 训练数据，不是用来评测的

判断依据：优先看README中的关键词（bench/benchmark/测试/评估/evaluation等）和语境。如果关键词只是顺带提及（如"我们在benchmark上测试了效果"），应判为非榜单。

2. **Agent类型**（六选一）：
- 代码/机器人: 要求输出代码或控制机器人。注意：代码+机器人算一个标签
- 多模态: 每条数据都包含图片/音频/视频（如每行都有screenshot列）
- 通用: 纯文本任务（对话、问答、推理、浏览器操作等）
...
混合规则：
- 代码 + 机器人 → 不算混合，归"代码/机器人"
- 通用 + 多模态 → 混合可用（说明"通用+多模态"）
- 通用 + 代码/机器人 → 混合可用（说明"通用+代码/机器人"）
- 代码/机器人 + 多模态（无通用） → 混合不可用（说明"代码/机器人+多模态"）
...
榜单原句：如果是benchmark榜单或其他榜单，提取关键词所在的原句（最多200字符）；否则填"/"
...
if __name__ == '__main__':
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            info = extract_info(seq, url, dataset_id, None, '')
        results.append(info)
        time.sleep(req_config['sleep_between_items'])
